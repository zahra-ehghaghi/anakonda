...
class TaskController:
    ...
    def get_tasks():
        try:
            ...
            return jsonify(state=500, code=103)

        try:
            tasks = Task.query.all()
        except:
            return jsonify(state=500, code=106)
    ...
    def get_task(task_id):
        ...
        try:
            ...
            return jsonify(state=500, code=106)
        if task is None:
            return jsonify(state=404, code=107)

        return jsonify(state=tasks_schema.dump(task))
    ...
    def create_task():
        ...
        try:
            ...
            return jsonify(state=400, code=102)
        if len(request_data["name"]) > 32 or not request_data["name"].islower():
            return jsonify(state=400, code=104)
        ...
        if (
            ...
            return jsonify(state=400, code=104)
        if request_data["runtime"] not in Config.AVAILABLE_RUNTIME:
            return jsonify(state=400, code=108)
        ...
        try:
            ...
            db.session.rollback()
            return jsonify(state=500, code=106)
        return jsonify(state=task_schema.dump(task), status=201)
    ...
    def update_task(task_id):
        ...
        if task.status != "new":
            return jsonify(state=400, code=109)
        ...
        task.last_update_at = datetime.now()
        ...
        try:
            ...
            db.session.rollback()
            return jsonify(state=500, code=106)
        return jsonify(state=task_schema.dump(task))
